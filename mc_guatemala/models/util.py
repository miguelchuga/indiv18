...
def num_a_letras_factura(num,moneda, completo=True):
    en_letras = {
        '0': 'cero',
        '1': 'uno',
        '2': 'dos',
    ...
    if moneda == 'USD':
        ...
        _moneda = 'quetzales '
    if decimal == 0:
        letras = '%s %s exactos' % (_moneda,num_en_letras)
    else:
        letras = '%s %s con %s/100' % (num_en_letras,_moneda,decimal)
...
def num_a_letras(num, completo=True):
    ...
    if decimal == 0:
        letras = 'quetzales %s exactos' % num_en_letras
...
def num_a_letras_dolar(num, completo=True):
    ...
    if decimal == 0:
        letras = 'dolares %s exactos' % num_en_letras
    else:
#        letras = '%s con %s centavos.' % (num_en_letras, decimal)
        letras = '%s dolares con %s/100' % (num_en_letras, decimal)
...
def fecha_a_letras(fecha):
    mes=mes_a_letras(fecha.month)
    return ('Guatemala '+str(fecha.day)+" de "+mes+" del "+str(fecha.year))
